...
class AudioDataset(nn.Module):
    def __init__(self, partition, instruments, sr, channels, out_channels, random_hops, hdf_dir, shapes, audio_transform=None, in_memory=False):
        ...
        with h5py.File(self.hdf_dir, "r") as f:
            ...
            lengths = [(l // self.shapes['output_len']) + 1 for l in lengths]
    ...
    def __getitem__(self, idx):
        ...
        if self.random_hops:
            start_target_pos = np.random.randint(0, max(target_length - self.shapes['output_len'] + 1, 1))
        else:
            # Map item index to sample position within song
            start_target_pos = idx * self.shapes['output_len']
        start_pos = start_target_pos - self.shapes['start_frame']
        if start_pos < 0:
            ...
            pad_front = 0
        end_pos = start_target_pos - self.shapes['start_frame'] + self.shapes['input_len']
        ...
        if self.out_channels == 1:
            ...
            for k in sources.keys():
                targets = np.zeros([self.out_channels, self.shapes['output_len']], dtype=np.float32)
                targets[idx_temp] = sources[k]
                idx_temp += 1
        return torch.tensor(audio), torch.tensor(targets)
